...
## Primary Objectives
- Describe JavaScript -- what is it?
- Understand some key differences between JS and Ruby
- Review the most basic JS fundamentals
...
```
  ...
  * Another point to note is that _functions without an explicit return value will return `undefined`_. **PLEASE BURN THIS INTO YOUR MEMORY**. The two exceptions to this rule are with one-line, single expression arrow functions, and the constructor function invocation pattern (using the `new` keyword -- don't worry, we'll get there...!).
